	...
	function timeline($user, $limit=5, $thumbnail=false, $cache_override=false) {
		...
		// Get new results
			...
			for ($i=0; $i<$max; $i++) {
				...
				$return = $return . '<tr><td id="tweet_'.$tweet_id.'" class="'.$color[$c].'">';
				$return = $return . '<p>'.$thumb.$tweet.'</p>';
/*				$return = $return . '<span class="tweet_source">Posted <a href="http://twitter.com/#!/'.$user
				.'/status/'.$tweet_id.'">'.$date.'</a>'; */
				$return = $return . '</td></tr>';
				$return = $return . "\n";
				$return = $return . "\n";
	...
	function search ($term, $limit=5, $thumbnail='no_thumbnail', $cache_override=false, $lang='en') {
		...
		// Get new results
			...
			foreach($twitter['results'] as $result) {
				...
				$return = $return . '<tr><td id="result_'.$i.'" class="'.$color[$c].'" >'.$n;
				$return = $return . $thumb;
